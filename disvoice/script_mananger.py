...
def script_manager(args, feature_method):
    ...
    features = extract_features(
        feature_method, audio, file_features, static, plots, fmt
    )
...
def extract_features(feature_method, audio, file_features, static, plots, fmt):
    ...
    if audio.find(".wav") != -1 or audio.find(".WAV") != -1:
        if fmt == "kaldi":
            feature_method.extract_features_file(
                audio, static=static, plots=plots, fmt=fmt, kaldi_file=file_features
            )
        else:
            features = feature_method.extract_features_file(
                audio, static=static, plots=plots, fmt=fmt
            )

    else:
        if fmt == "kaldi":
            feature_method.extract_features_path(
                audio, static=static, plots=plots, fmt=fmt, kaldi_file=file_features
            )
        else:
            features = feature_method.extract_features_path(
                audio, static=static, plots=plots, fmt=fmt
            )
    return features
...
def check_paramters(args):
    ...
    if args[3] == "false" or args[3] == "False":
        ...
        raise ValueError(
            args[3]
            + " is not a valid argument for <static>. It should be only True or False"
        )
    ...
    if args[4] == "false" or args[4] == "False":
        ...
        raise ValueError(
            args[4]
            + " is not a valid argument for <plots>. It should be only True or False"
        )
    ...
    if (
        ...
        raise ValueError(
            args[5]
            + " is not a valid argument for <format>. It should be only csv, txt, npy, kaldi, or torch"
        )
