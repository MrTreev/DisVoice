...
from ..disvoice_utils import dynamic2statict, get_dict, save_dict_kaldimat
from ..praat import praat_functions
from ..script_mananger import script_manager
from .phonation_functions import (APQ, PPQ, get_log_energy, jitter_env,
                                  shimmer_env)
...
class Phonation:
    ...
    def extract_features_file(
        ...
        for l in range(nF):
            if F0[l] == 0:
                continue
            data_frame = data_audio[
                int(l * size_stepS) : int(l * size_stepS + size_frameS)
            ]
        ...
        if static:
            ...
            if len(Shimmer) == len(apq):
                feat = np.vstack(
                    (DF0[5:], DDF0[4:], Jitter[6:], Shimmer[6:], apq[6:], ppq, logE[6:])
                ).T
            else:
                feat = np.vstack(
                    (
    ...
    def get_F0(self, audio, fs, data_audio, size_stepS):
        if self.pitch_method == "praat":
            ...
            F0, _ = praat_functions.decodeF0(
                temp_filename_f0, len(data_audio) / float(fs), self.size_step
            )
...
if __name__ == "__main__":
    ...
    if len(sys.argv) != 6:
        print(
            "python phonation.py <file_or_folder_audio> <file_features> <static (true, false)> <plots (true,  false)> <format (csv, txt, npy, kaldi, torch)>"
        )
        sys.exit()
