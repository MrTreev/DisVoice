...
from .utils_gci import (GetLPCresidual, RESON_dyProg_mat, calc_residual,
                        create_continuous_smooth_f0, get_MBS,
                        get_MBS_GCI_intervals, search_res_interval_peaks)
...
def se_vq_varf0(x, fs, f0=None):
    ...
    res = GetLPCresidual(
        x, winLen * fs / 1000, winShift * fs / 1000, LPC_ord, VUV_inter
    )  # Get LP residual
    ...
    [GCI_N, GCI_relAmp] = search_res_interval_peaks(
        res, interval, Ncand, VUV_inter
    )  # Find residual peaks
    if len(np.asarray(GCI_N).shape) > 1:
        GCI = RESON_dyProg_mat(
            GCI_relAmp, GCI_N, F0mean, x, fs, trans_wgt, relAmp_wgt, plots=False
...
def get_vq_params(gf, gfd, fs, GCI):
    ...
    for n in range(len(GCI)):
        ...
        if start != stop and len(gf_comb) > 1:
            line = np.interp(
                np.arange(stop - start), np.linspace(0, stop - start, 2), gf_comb
            )
