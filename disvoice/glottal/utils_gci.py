...
from scipy.signal import butter, buttord, filtfilt, lfilter, lfiltic, medfilt
...
def create_continuous_smooth_f0(F0, VUV, x):
    ...
    for j in range(n_seg):
        f0_int_cur = np.interp(
            np.arange(start[j], stop[j]),
            [start[j], stop[j]],
            [F0[start[j]], F0[stop[j] + 1]],
        )
        f0_inter[start[j] + 1 : stop[j] + 1] = f0_int_cur
    f0_inter = smooth(medfilt(f0_inter, med_len), sm_len)
    f0_samp = np.interp(
        np.arange(len(x)), np.linspace(0, len(x), len(f0_inter)), f0_inter
    )
...
def RCVD_reson_GCI(res, fs, F0mean):
    ...
    rep = filtfilt(
        [1.0, 0.0, 0.0],
        [1.0, -2 * Rho * np.cos(Phi), Rho**2],
        res,
        padtype="odd",
        padlen=9,
    )  # Filter forwards and backwards to have zero-phase
...
def RESON_dyProg_mat(
    GCI_relAmp, GCI_N, F0mean, x, fs, trans_wgt, relAmp_wgt, plots=True
):
...
def calc_residual(x, x_lpc, ord_lpc, GCI):
    ...
    for n in range(len(GCI) - 1):
        ...
        try:
            ...
            vector_res[start : start + len(residual_win)] = (
                vector_res[start : start + len(residual_win)] + residual_win
            )
