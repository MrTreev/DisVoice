...
from ..disvoice_utils import (dynamic2statict_artic, fill_when_empty, get_dict,
                              save_dict_kaldimat)
from ..praat import praat_functions
from ..script_mananger import script_manager
from .articulation_functions import (extract_transitions,
                                     get_transition_segments)
...
class Articulation:
    ...
    def plot_art(self, data_audio, fs, F0, F1, F2, segmentsOn, segmentsOff):
        ...
        for j in range(1, len(segmentsOn) - 1):
            f, (a0, a1) = plt.subplots(
                2, 1, gridspec_kw={"height_ratios": [1, 3]}, sharex=True, figsize=(5, 4)
            )
            ...
            a1.imshow(
                np.log10(np.abs(np.flipud(spec))),
                extent=[0, 0.08, 1, 4000],
                aspect="auto",
                cmap=plt.cm.viridis,
                vmax=np.log10(np.abs(spec).max()),
                vmin=np.log10(np.abs(spec).min()),
        ...
        for j in range(1, len(segmentsOff) - 1):
            ...
            a0.plot(
                t[640:], segmentsOff[j][640:], color="k", alpha=0.5, label="unvoiced"
            )
            a0.grid()
    ...
    def extract_features_file(
        ...
        F0, segmentsOn, segmentsOff = self.extract_transition_segments(
            fs, data_audio, size_stepS
        )

        BBEon, MFCCon = extract_transitions(
            segmentsOn, fs, size_frameS, size_stepS, self.nB, self.nMFCC
        )
        BBEoff, MFCCoff = extract_transitions(
            segmentsOff, fs, size_frameS, size_stepS, self.nB, self.nMFCC
        )

        DMFCCon = np.asarray(
            [np.diff(MFCCon[:, nf], n=1) for nf in range(MFCCon.shape[1])]
        ).T
        DDMFCCon = np.asarray(
            [np.diff(MFCCon[:, nf], n=2) for nf in range(MFCCon.shape[1])]
        ).T

        DMFCCoff = np.asarray(
            [np.diff(MFCCoff[:, nf], n=1) for nf in range(MFCCoff.shape[1])]
        ).T
        DDMFCCoff = np.asarray(
            [np.diff(MFCCoff[:, nf], n=2) for nf in range(MFCCoff.shape[1])]
        ).T
        ...
        if static:
            feat_mat = dynamic2statict_artic(
                [
                    BBEon,
                    MFCCon,
                    DMFCCon,
...
if __name__ == "__main__":
    ...
    if len(sys.argv) != 6:
        print(
            "python articulation.py <file_or_folder_audio> <file_features> <static (true, false)> <plots (true,  false)> <format (csv, txt, npy, kaldi, torch)>"
        )
        sys.exit()
