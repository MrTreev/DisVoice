...
def extract_transitions(
    segments, fs, size_frameS, size_stepS, nB=22, nMFCC=12, nfft=2048
):
    frames = []
    ...
    for j in range(len(segments)):
        if len(segments[j]) > size_frameS:
            ...
            for iF in range(nF):
                frames.append(
                    np.hamming(size_frameS)
                    * segments[j][
                        int(iF * size_stepS) : int(iF * size_stepS + size_frameS)
                    ]
                )
