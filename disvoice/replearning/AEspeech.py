...
class AEspeech:
        ...
        if model == "CAE":
            ...
            if torch.cuda.is_available():
                ...
                self.AE.load_state_dict(
                    torch.load(PATH + "/" + str(units) + "_CAE.pt", map_location="cpu")
                )
        elif model == "RAE":
            self.AE = RAEn(units)
            if torch.cuda.is_available():
                ...
                self.AE.load_state_dict(
                    torch.load(PATH + "/" + str(units) + "_RAE.pt", map_location="cpu")
                )

        else:
            raise ValueError(
                "Model " + model + " is not valid. Please choose only CAE or RAE"
            )
    ...
    def compute_spectrograms(self, wav_file):
        ...
        if nf > 0:
            ...
            raise ValueError(
                "WAV file is too short to compute the Mel spectrogram tensor"
            )
    ...
    def compute_global_features(self, wav_directory, stack_feat=False):
        ...
        if stack_feat:
            feat_all = np.zeros(
                (
                    len(hf),
                    len(self.feat_names_bottle_all) + len(self.feat_names_error_all),
                )
            )

        for i, wav_file in enumerate(hf):
            try:
                bottle = self.compute_bottleneck_features(
                    wav_directory + wav_file, True
                )
                bottle_feat[i, :] = np.hstack(
                    (
                        np.mean(bottle, 0),
                        np.std(bottle, 0),
                        st.skew(bottle, 0),
                        st.kurtosis(bottle, 0),
                    )
                )
                error = self.compute_rec_error_features(wav_directory + wav_file, True)
                error_feat[i, :] = np.hstack(
                    (
                        np.mean(error, 0),
                        np.std(error, 0),
                        st.skew(error, 0),
